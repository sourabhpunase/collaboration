...
class AuthService {
  ...
  async deleteUser(userId) {
    try {
      // Clean up user's projects and related data first
      ...
      for (const project of userProjects) {
        ...
        Array.from(this.stagedChanges.keys())
          .filter(key => this.stagedChanges.get(key).projectId === project.id)
          .forEach(key => this.stagedChanges.delete(key));
        Array.from(this.comments.keys())
          .filter(key => this.comments.get(key).projectId === project.id)
          .forEach(key => this.comments.delete(key));
        this.projectHistory.delete(project.id);
      }
      ...
      Array.from(this.projects.values()).forEach(project => {
        ...
        if (memberIndex > -1) {
          ...
          project.memberNames.splice(memberIndex, 1);
        }
      });
      
      // Clean up invitations
      Array.from(this.invitations.keys())
        .filter(key => {
          const inv = this.invitations.get(key);
          return inv.inviterId === userId || inv.inviteeId === userId;
        })
        .forEach(key => this.invitations.delete(key));
      
      // Try to delete related database records first
      try {
        // Delete from documents table if it exists
        await axios.delete(
          `${this.baseURL}/rest/v1/documents?created_by=eq.${userId}`,
          { headers: { ...this.headers, 'Prefer': 'return=minimal' }, httpsAgent: this.agent }
        );
      } catch (dbError) {
        console.log('No documents to delete or table does not exist');
      }
      ...
      // Always try to disable user if deletion fails
      try {
        await axios.put(
          `${this.baseURL}/auth/v1/admin/users/${userId}`,
          { 
            ...
            user_metadata: { disabled: true, disabled_reason: 'Account deleted by admin' }
          },
          { headers: this.headers, httpsAgent: this.agent }
        );
        return { success: true, message: 'User account disabled (deletion failed due to existing content)' };
      } catch (disableError) {
        throw new Error(`Cannot delete user: ${error.response?.data?.message || error.message}`);
      }
    }
  }
